...
for i in range(len(l_grid)):
    ...
    # J_r_res = Omega_b/GM**2 * 0.5*(abs(Lz_grid) + np.sqrt(abs(Lz_grid)**2+4*GM*b))**3 - 0.5*(1 + abs(Lz_grid)/np.sqrt(Lz_grid**2 + 4*GM*b))*np.sign(Lz_grid)
...
# ax.scatter(Lz0, E0/1e5, marker='x', c='k')
...
for i in range(len(l_grid)):
    ...
    # axs[i,1].legend(loc=2, fontsize=16, handlelength=0)
...
# def calc_G_discrete(J_r, J_phi, l, m, Delta_J=1):
        
#     L = abs(J_phi)
#     L_plus_Delta_J = abs(J_phi + Delta_J)
    
#     dOmegar_dJr = (calc_Omega_r(J_r+Delta_J, L) - calc_Omega_r(J_r, L))/Delta_J
#     dOmegar_dJphi = (calc_Omega_r(J_r, L_plus_Delta_J) - calc_Omega_r(J_r, L))/Delta_J
    
#     dOmegaphi_dJr = (calc_Omega_phi(J_r+Delta_J, L, J_phi) - calc_Omega_phi(J_r, L, J_phi))/Delta_J
#     dOmegaphi_dJphi = (calc_Omega_phi(J_r, L_plus_Delta_J, J_phi+Delta_J) - calc_Omega_phi(J_r, L, J_phi))/Delta_J

#     G = l**2 * dOmegar_dJr + m**2 * dOmegaphi_dJphi + m*l*(dOmegar_dJphi + dOmegaphi_dJr)

#     return G


# def calc_G_res(J_phi, J_r, l, m):
#     L = abs(J_phi)
...
#     alpha = 0.5*(1 + L/np.sqrt(L**2 + 4*GM*b))
#     G = dOmegar_dJr * ((l + m*alpha*np.sign(J_phi))**2 - 2/3 * m**2 * alpha*(1-alpha)*(2*alpha-1) * (alpha*np.sign(J_phi) + l/m)**(1/3) * GM**(2/3)/(Omega_b**(1/3)*L))
